		...
		for (ul i = 0; i < numThreads; i++) {
			...
			*/
			...
			for (ul j = 0; j < f.n_rows; j++) {
				...
				PR_L->at(j) = (pFw->getQls()) * (Ar.st()) + (pFw->getQli()) * (A_2.st());
				D_L->at(j) = (dFw->getDF3ls()) * (Ar.st()) + (dFw->getDF3li()) * (A_2.st());

				/*
				arma::cx_mat* tPR_L = new arma::cx_mat(1, 1);
				arma::cx_mat* tD_L = new arma::cx_mat(1, 1);

				//tPR_L->at(0,0) = pFw->getQls() * Ar.st() + pFw->getQli() * A_2.st();
				//tD_L->at(0,0) = (dFw->getDF3ls()) * (Ar.st()) + (dFw->getDF3li()) * (A_2.st());
			...
			/*
			std::vector<std::string>  ResultTags = {"PR_L", "D_L"};
			std::vector<arma::cx_mat> ResultData = {PR_L, D_L};
			...
			//std::vector *temp = PR_L.get();
			//std::cout << "PR_L:"<< temp <<std::endl;
